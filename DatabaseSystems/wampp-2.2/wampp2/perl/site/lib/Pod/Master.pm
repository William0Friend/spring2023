#!Perl
# pod2html doesn't resolve C< L<Update|"Update"> >
# or C< L<"Update"> > into a link ~ DAMMIT!!!
# Even if its code you still have to use E<gt> E<lt>
# i'll have to check out Marek::Pod::Html
# or write one from Pod::Select or Pod::Pom (fsck Pod::Tree)
...
(the rest are either automagically generated ):
...
sub UpdatePOD {
  ...
  my $libPods = 'perlfunc:perlguts:perlvar:perlrun:perlopt';
  my $BackLink = $self->{pod2html}{backlink};
  my $css = $self->{pod2html}{css} || "Active.css";
  my $p2quiet = $self->{pod2html}{quiet};
  my $p2header = $self->{pod2html}{header};
  my $p2verbose = $self->{pod2html}{verbose};
  (my $site = $SiteLib) =~ s!\\lib$!!;
  my $PodPath = join':',
    map{
      s{\Q$InstallPrefix\E}{};
      canonpath("./$_");
    }
      ($ScriptDir,$PrivLib,$site);
...
# my $RelPath = abs2rel( catdir( (splitpath$InFile)[1,2] ), $InstallPrefix );
      ...
      my $HtmlRoot = catdir map { $_ ? '..' : $_ } 
	splitdir((splitpath$RelPath)[1]);
      my $OutFile = catdir $OutDir, $RelPath;
      $OutFile =~ s!\.([^\.]+)$!.html!;
      
      my $HtmlDir = catdir( ( splitpath($OutFile) )[0,1] );
      (my $installhtml = $InstallHtmlDir) =~ s!\\!/!g;
      my $style = 'file://' . $installhtml . '/'. $css;
      my $root = "file://$installhtml";
      my @args = (
		  "--htmldir=$HtmlDir",
#                  "--htmlroot=$HtmlRoot",
                  "--htmlroot=$root",
...
Re-Creates index.html and Active.css if they're missing,
...
sub UpdateTOC {
  eval q[use ActivePerl::DocTools::TOC::HTML::Podmaster; 
	 ActivePerl::DocTools::TOC::HTML::Podmaster::WriteTOC() ];
  ...
  if(not -e $MasterCss and $MasterCss =~ /Active\.css/){
...
go to <a target=_self href='podmaster.toc.html'>toc</a>(the big one)
  ...
  open(OUT,'>podmaster.miniframe.html') 
    or die "oops podmaster.miniframe.html $!";
...
<h1>Table of Contents</h1>
  ...
  if( $boiler
    ...
    $ret.= ActivePerl::DocTools::TOC::HTML->boiler_links();
  }
